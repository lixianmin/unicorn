...
namespace Metadata
{
    public class XmlMetadataSerializer
    {
        public XmlMetadataSerializer()
        {
            ...
            // Client项目中定义了一个abstract的泛型基类: Template2<T>
            var extraTypes = (from type in EditorMetaTools.GetMetaTypes() where !type.IsAbstract select type.RawType).ToArray();
        ...
        private void _OnDeserializeUnknownElement(object sender, XmlElementEventArgs e)
        {
            ...
            if (null != target)
            {
                ...
                foreach (var member in typeMembers)
                {
                    ...
                    {
                        Logo.Error(
                            "unknown={0}, maybe={1}, LineNumber={2} \n\n ObjectBeingDeserialized={3} \n\n clipText={4}"
                            , elementName
                            , memberName
